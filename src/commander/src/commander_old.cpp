...
void createPeduncle(geometry_msgs::Pose target_pose){
  ...
  collision_object.primitive_poses.push_back(peduncle_pose);
...
int main(int argc, char** argv)
{
  ...
  target_pose.orientation.y = 0.2;
  target_pose.orientation.z = 0.1;
  target_pose.orientation.w = 1.0;
  target_pose.position.x = 0.4;
  target_pose.position.y = 0;
  target_pose.position.z = 0.3;
   
  createPeduncle(target_pose);
  // moveToPoseWithEndEffector()
  moveToPose(target_pose); // reset to home
  ...
  // moveToPoseWithConstraint(target_pose);
